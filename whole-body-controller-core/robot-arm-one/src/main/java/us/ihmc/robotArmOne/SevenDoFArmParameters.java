...
public class SevenDoFArmParameters {
	/**
	 * Enum declaring the 7-DoFs of a robot arm in order from the base to the
	 * end-effector.
	 */
	public enum SevenDoFArmJointEnum {
		...
		/**
		 * @return human-readable name to use when creating the joint corresponding to
		...
		public String getChildLinkName() {
			if (!jointChildLinkNames.containsKey(this))
				throw new RuntimeException(
						"No name has been registered for the child link of the joint: " + getJointName());
		...
		public double getChildLinkMass() {
			if (!jointChildLinkMasses.containsKey(this))
				throw new RuntimeException(
						"No mass has been registered for the child link of the joint: " + getJointName());
			return jointChildLinkMasses.get(this);
		}

		/**
		 * @return the center of mass of the link that is attached to this joint. It is
		 *         expressed with respect to the parent joint.
		 */
		public Vector3D getChildLinkCoM() {
			if (!jointChildLinkCoMs.containsKey(this))
				throw new RuntimeException(
						"No CoM has been registered for the child link of the joint: " + getJointName());
		...
		public Matrix3D getChildLinkInertia() {
			if (!jointChildLinkInertias.containsKey(this))
				throw new RuntimeException(
						"No inertia has been registered for the child link of the joint: " + getJointName());
		...
		public Graphics3DObject getChildLinkGraphics() {
			if (!jointChildLinkGraphics.containsKey(this))
				throw new RuntimeException(
						"No graphics has been registered for the child link of the joint: " + getJointName());
	...
	public static final Matrix3D armLinkInertia = RotationalInertiaCalculator
			.getRotationalInertiaMatrixOfSolidCylinder(armLinkMass, armLinkRadius, armLinkLength, Axis3D.Z);
	...
	public static final Matrix3D handInertia = RotationalInertiaCalculator
			.getRotationalInertiaFromRadiiOfGyration(handMass, 0.08, 0.08, 0.08);

	public static final EnumMap<SevenDoFArmJointEnum, Vector3DReadOnly> jointAxes = new EnumMap<>(
			SevenDoFArmJointEnum.class);
	public static final EnumMap<SevenDoFArmJointEnum, Vector3D> jointOffsets = new EnumMap<>(
			SevenDoFArmJointEnum.class);
	public static final EnumMap<SevenDoFArmJointEnum, Double> jointLowerLimits = new EnumMap<>(
			SevenDoFArmJointEnum.class);
	public static final EnumMap<SevenDoFArmJointEnum, Double> jointUpperLimits = new EnumMap<>(
			SevenDoFArmJointEnum.class);
	public static final EnumMap<SevenDoFArmJointEnum, String> jointChildLinkNames = new EnumMap<>(
			SevenDoFArmJointEnum.class);
	public static final EnumMap<SevenDoFArmJointEnum, Double> jointChildLinkMasses = new EnumMap<>(
			SevenDoFArmJointEnum.class);
	public static final EnumMap<SevenDoFArmJointEnum, Vector3D> jointChildLinkCoMs = new EnumMap<>(
			SevenDoFArmJointEnum.class);
	public static final EnumMap<SevenDoFArmJointEnum, Matrix3D> jointChildLinkInertias = new EnumMap<>(
			SevenDoFArmJointEnum.class);
	public static final EnumMap<SevenDoFArmJointEnum, Graphics3DObject> jointChildLinkGraphics = new EnumMap<>(
			SevenDoFArmJointEnum.class);
	...
	static {
		...
		jointChildLinkGraphics.put(SevenDoFArmJointEnum.shoulderPitch,
				createArmLinkGraphics(armLinkLength, armLinkRadius, YoAppearance.CadetBlue()));
		jointChildLinkGraphics.put(SevenDoFArmJointEnum.elbowPitch,
				createArmLinkGraphics(armLinkLength, armLinkRadius, YoAppearance.DarkRed()));
	...
	public static Graphics3DObject createArmLinkGraphics(double length, double radius,
