...
public class RobotArmOneController implements RobotController {
	...
	/**
	 * Proportional gain for each joint. {@code YoDouble}s are used instead of
	 * simple {@code double} so they can be viewed and modified via the Simulation
	 * Construction Set.
	 */
	private final EnumMap<SevenDoFArmJointEnum, YoDouble> kps = new EnumMap<>(SevenDoFArmJointEnum.class);
	/**
	 * Derivative gain for each joint. {@code YoDouble}s are used instead of simple
	 * {@code double} so they can be viewed and modified via the Simulation
	 * Construction Set.
	 */
	...
	/**
	 * The error in position for the tracking of each joint. {@code YoDouble}s are
	 * used instead of simple {@code double} so they can be monitored via the
	 * Simulation Construction Set.
	 */
	private final EnumMap<SevenDoFArmJointEnum, YoDouble> positionErrors = new EnumMap<>(SevenDoFArmJointEnum.class);
	/**
	 * The error in velocity for the tracking of each joint. {@code YoDouble}s are
	 * used instead of simple {@code double} so they can be monitored via the
	 * Simulation Construction Set.
	 */
	...
	/**
	 * This method allows to manage the initialization part needed to get the
	 * controller ready to go. It is NOT called by default by the simulation, it has
	 * to be called at the desired location in the configuration of the robot and
	 * control environment.
	 */
	@Override
