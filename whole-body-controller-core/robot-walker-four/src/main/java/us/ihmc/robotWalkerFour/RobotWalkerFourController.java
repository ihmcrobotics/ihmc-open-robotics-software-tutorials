...
/**
 * The objective here is to implement small controller that uses the IHMC
 * whole-body controller to make the M2 robot achieve quasi-static walking.
 * <p>
 * In this controller, we use a set of additional features with respect to the
 * previous examples:
 * <ul>
 * <li>Requesting the controller core to use a rigid-body for supporting the
 * body weight using the new command: {@code PlaneContactStateCommand}.
 * <li>controlling the robot center of mass position with the command:
 * {@code CenterOfMassFeedbackControlCommand}.
 * <li>The direct control of a rigid-body acceleration using:
 * {@code SpatialAccelerationCommand}.
 * </ul>
 * Finally, this controller also introduces the state machine framework:
 * {@code StateMachine}.
 * </p>
 */
public class RobotWalkerFourController implements RobotController {
	...
	/**
	 * We use this registry to keep track of the controller variables which can then
	 * be viewed in Simulation Construction Set.
	 */
	...
	/**
	 * This is the IHMC whole-body controller core which is used at the core of the
	 * walking controller on Atlas and Valkyrie.
	 */
	private final WholeBodyControllerCore wholeBodyControllerCore;

	/**
	 * We define as an enum the list of possible states that the state machine will
	 * be allowed to go through.
	 */
	private enum WalkingStateEnum {
		/**
		 * The simulation starts with the robot standing. It will switch to the initial
		 * transfer only when the variable {@code walk} is set to {@code true}.
		 */
		STANDING,
		/**
		 * This is the double support phase that moves the center of mass from the right
		 * foot to the left foot.
		 */
		TRANSFER_TO_LEFT,
		/**
		 * This is the double support phase that moves the center of mass from the left
		 * foot to the right foot.
		 */
		TRANSFER_TO_RIGHT,
		/**
		 * This is the single support phase during which the center of mass stays still
		 * while the right is swinging to the desired footstep location.
		 */
		LEFT_SUPPORT,
		/**
		 * This is the single support phase during which the center of mass stays still
		 * while the left is swinging to the desired footstep location.
		 */
		RIGHT_SUPPORT
	};

	/**
	 * The finite state machine to which we register a set of specialized
	 * controllers and a set of transitions to go from one controller to another.
	 */
	private final StateMachine<WalkingStateEnum, State> stateMachine;
	/**
	 * This variable stores the current simulation time and is updated by the
	 * simulation.
	 */
	...
	/**
	 * The elevator is a massless, sizeless rigid-body fixed in world to which the
	 * first joint of the robot is attached. The name comes from the use of this
	 * rigid-body to add the gravity effect to the robot by making it accelerate
	 * like an elevator when it starts moving. However, this elevator is always
	 * fixed in world with no velocity.
	 */
	private final RigidBodyBasics elevator;
	/**
	 * We will use a single instance of the controller core command for convenience.
	 * Note that in this example, only the Inverse Dynamics control mode will be
	 * demonstrated.
	 */
	...
	/**
	 * We will define only one set of gains for controlling the whole robot. This is
	 * not ideal but is enough for this example.
	 */
	private final DefaultYoPIDSE3Gains gains = new DefaultYoPIDSE3Gains("gains", GainCoupling.XYZ, false, registry);

	public RobotWalkerFourController(RobotWalkerFour robotWalkerFour, double controlDT, double gravityZ,
	...
	private WholeBodyControllerCore createControllerCore(double controlDT, double gravityZ,
			YoGraphicsListRegistry yoGraphicsListRegistry) {
		...
		// properly.
		WholeBodyControlCoreToolbox toolbox = new WholeBodyControlCoreToolbox(controlDT, gravityZ, rootJoint,
				jointsArray, robotWalkerFour.getCenterOfMassFrame(), controllerCoreOptimizationSettings,
		...
		/*
		 * Finally, we register all the commands that we will use in this controller,
		 * i.e. commands for the feet that we'll for the swing, an orientation command
		 * for the pelvis to keep it level to the ground, and the command for
	...
	private StateMachine<WalkingStateEnum, State> createStateMachine() {
		...
		/*
		 * Then we get to the point where actually create the state and the transitions.
		 * In this example, we will only use what is called here "done transitions".
		 * These transitions get triggered as a state reports that it is done, when
		 * triggered the state machine goes to the next state.
		 */
		// Here we setup the STANDING state. When done, the state machine will
		// transition to the TRANSFER_TO_LEFT state.
		factory.addStateAndDoneTransition(WalkingStateEnum.STANDING, new StandingState(),
				WalkingStateEnum.TRANSFER_TO_LEFT);

		// Here using the transitions, we create a cycle that will result in having the
		// robot walking indefinitely.
		factory.addStateAndDoneTransition(WalkingStateEnum.TRANSFER_TO_LEFT, new TransferState(RobotSide.LEFT),
				WalkingStateEnum.LEFT_SUPPORT);
		factory.addStateAndDoneTransition(WalkingStateEnum.LEFT_SUPPORT, new SingleSupportState(RobotSide.LEFT),
				WalkingStateEnum.TRANSFER_TO_RIGHT);
		factory.addStateAndDoneTransition(WalkingStateEnum.TRANSFER_TO_RIGHT, new TransferState(RobotSide.RIGHT),
				WalkingStateEnum.RIGHT_SUPPORT);
		factory.addStateAndDoneTransition(WalkingStateEnum.RIGHT_SUPPORT, new SingleSupportState(RobotSide.RIGHT),
				WalkingStateEnum.TRANSFER_TO_LEFT);
	...
	/**
	 * This time, the {@code doControl} method is rather empty as most of the
	 * controller is actually implemented in the different states declared further
	 * down.
	 */
	@Override
	public void doControl() {
		...
		/*
		 * Here we request the state machine to call the doAction() method of the active
		 * state and to check if the transition to the next state should be engaged. See
		 * further below for the implementation of the doAction() method for each state.
		 */
		stateMachine.doActionAndTransition();

		/*
		 * As the pelvis is to kept level to the ground independently to the active
		 * state, we can simply setup the command here.
		 */
		OrientationFeedbackControlCommand pelvisOrientationCommand = new OrientationFeedbackControlCommand();
		pelvisOrientationCommand.setControlMode(WholeBodyControllerCoreMode.INVERSE_DYNAMICS); // sets control mode to
																								// inverse dynamics
		...
		JointDesiredOutputListReadOnly outputForLowLevelController = wholeBodyControllerCore
				.getOutputForLowLevelController();
	...
	/**
	 * A {@code CenterOfMassCommand} is created using the new calculated
	 * {@code centerOfMassPosition} and adds the command to the controller core.
	 * 
	 * @param centerOfMassPosition refers to newly calculated position of the center
	...
	public void sendCenterOfMassCommand(FramePoint3DReadOnly centerOfMassPosition) {
		...
		centerOfMassCommand.setInverseDynamics(centerOfMassPosition, feedForwardLinearVelocity,
	...
	/**
	 * During the standing state, the center of mass is kept at a constant position
	 * right between the feet and the feet are both used for support.
	 */
	private class StandingState implements State {
		...
		public boolean isDone(double timeInState) {
			/*
			 * As soon as the walk variable is set to true via the simulation GUI, the state
			 * machine will leave this state causing the robot to start walking.
			 */
	...
	/**
	 * During the transfer state, the robot is still in double support but the
	 * center of mass is now moving to be above the leading foot.
	 */
	private class TransferState implements State {
		...
		/**
		 * We use a 5th order polynomial to smooth out the velocity of the center of
		 * mass at the start and end of this state. This allows the robot to walk
		 * slightly faster without falling.
		 */
		...
		public TransferState(RobotSide transferToSide) {
			...
			centerOfMassTrajectory = new QuinticPolynomialTrajectoryGenerator(
					transferToSide.getCamelCaseName() + "CenterOfMassTrajectory", initialPositionProvider,
					initialVelocityProvider, finalPositionProvider, finalVelocityProvider, transferDuration, registry);
		}

		@Override
		public void onEntry() {
			/*
			 * This method is called once when this state becomes active. this is the
			 * perfect time to initialize the endpoints for the center of mass. It should
			 * start from wherever it is at the start of this state and end above the
			 * leading foot.
			 */
	...
	/**
	 * During the single support state, only one foot is in support while the other
	 * is swinging to the next footstep. The center of mass stays still.
	 */
	private class SingleSupportState implements State {
		...
		/**
		 * YoVariable for the next footstep position so we can monitor it during the
		 * simulation.
		 */
		...
		public SingleSupportState(RobotSide supportSide) {
			...
			PositionProvider initialPositionProvider = initialPosition -> initialPosition
					.setToZero(robotWalkerFour.getSoleFrame(swingSide));
			PositionProvider finalPositionProvider = finalPosition -> finalPosition.setIncludingFrame(footstepPosition);
			swingPositionTrajectory = new ParabolicPositionTrajectoryGenerator(swingSide + "Swing", WORLD_FRAME,
					swingDuration, initialPositionProvider, finalPositionProvider, groundClearance, registry);
		...
		public void doAction(double timeInState) {
			...
			/*
			 * Using the swing trajectory generator, we can compute the current desired
			 * position, velocity and acceleration for the swing foot.
			 */
	...
	/**
	 * Creates a spatial acceleration command to request a zero acceleration of the
	 * right or left foot.
	 * 
	...
	/**
	 * Creates the command for the controller core to indicate whether a foot is in
	 * support or not.
	 * 
	 * @param robotSide refers to which foot the command is to be created for.
	 * @param inSupport whether the foot should be used to support the robot weight
	 *                  or not.
