...
/**
 * In this simulation example, we will see how the IHMC whole-body controller
 * core can be used to implement a controller for achieving quasi-static walking
 * with the good old M2 robot model.
 */
public class RobotWalkerFourSimulation {
	public RobotWalkerFourSimulation() {
		...
		/*
		 * This time, we need to setup a ground contact model that will make the ground
		 * interact with the ground contact points of the robot. Without it, the robot
		 * would simply fall through the ground.
		 */
		...
		GroundContactModel linearGroundModel = new LinearGroundContactModel(simulatedRobot, groundKxy, groundBxy,
				groundKz, groundBz, simulatedRobot.getRobotsYoVariableRegistry());
		...
		RobotWalkerFourController robotArmController = new RobotWalkerFourController(robotWalker, controllerDT,
				gravityMagnitude, yoGraphicsListRegistry);
