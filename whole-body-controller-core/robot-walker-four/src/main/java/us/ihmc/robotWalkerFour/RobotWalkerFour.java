...
/**
 * In this class, we use a copy of the M2 robot model. M2 was a bipedal robot
 * developed at the MIT Leg Lab: <a href=
 * "http://www.ai.mit.edu/projects/leglab/robots/robots.html">website</a>.
 */
public class RobotWalkerFour {
	...
	/**
	 * As in the previous examples, we need this generator to create an inverse
	 * dynamics robot model that the whole-body controller core can work with.
	 */
	private final InverseDynamicsJointsFromSCSRobotGenerator inverseDynamicsRobot;
	/**
	 * A mapping to easily retrieve joints in the inverse dynamics robot given a
	 * joint from the simulated robot.
	 */
	private final SCSToInverseDynamicsJointMap jointMap;
	/**
	 * In this example, the center of mass reference frame will become handy as it
	 * allows to control the robot's center of mass position.
	 */
	private final ReferenceFrame centerOfMassFrame;
	/**
	 * We will define additional reference frames located at the center of the foot
	 * soles. This will allow to use them as reference when positioning the center
	 * of mass and will also be used as control frames during the swing phase.
	 */
	private final SideDependentList<ReferenceFrame> soleFrames = new SideDependentList<>();
	/**
	 * A {@code ContactablePlaneBody} allows to attach to a rigid-body a list of
	 * points that can be used to distribute the ground reaction forces on the foot
	 * when it is in support.
	 */
	...
	public RobotWalkerFour() {
		...
		centerOfMassFrame = new CenterOfMassReferenceFrame("centerOfMassFrame", WORLD_FRAME,
				inverseDynamicsRobot.getElevator());
		...
		for (RobotSide robotSide : RobotSide.values) {
			...
			transformToAnkle.getTranslation().setX(-footBack + 0.5 * footLength); // Offset to center the frame in the
																					// middle of the sole.
			ReferenceFrame soleFrame = ReferenceFrameTools.constructFrameWithUnchangingTransformToParent(soleFrameName,
					frameAfterAnkleJoint, transformToAnkle);
			soleFrames.put(robotSide, soleFrame);

			/*
			 * Now we define the corner points of the foot. They can be used to apply forces
			 * on the ground.
			 */
			...
			contactPoints.add(new Point2D(-0.5 * footLength, -0.5 * footWidth));
			contactPoints.add(new Point2D(-0.5 * footLength, 0.5 * footWidth));
			ContactablePlaneBody footContactableBody = new ListOfPointsContactablePlaneBody(foot, soleFrame,
					contactPoints);
	...
	/**
	 * Updates the state of the inverse dynamics robot model based on the state of
	 * the simulated robot.
	 */
	...
	/**
	 * Gets the reference frame which is origin is located at the robot's center of
	 * mass.
	 * <p>
	 ...
	 * <p>
	 * The floating joint is the 6 degrees of freedom joint that represent the
	 * unactuated virtual connection between the robot and the world.
	 * </p>
	 * 
	 ...
	 * 
	 * @param robotSide whether this method should return the left or right
	 *                  contactable foot.
	 ...
	 * 
	 * @param robotSide whether this method should return the left or right sole
	 *                  frame.
	...
	/**
	 * Finds the corresponding simulated joint to the given
	 * {@code inverseDynamicsJoint} and set its desired torque.
	 * 
