...
public class RobotArmTwoController implements RobotController {
	...
	/**
	 * Desired position for each joint. {@code YoDouble}s are used instead of simple
	...
	/**
	 * Desired velocity for each joint. {@code YoDouble}s are used instead of simple
	 * {@code double} so they can be monitored via the Simulation Construction Set.
	 */
	 ...
	 * @param yoGraphicsListRegistry in this example, we do not have use for this.
	 *                               In general, this registry allows to create
	 *                               dynamic 3D graphics in the simulation.
	 */
	public RobotArmTwoController(RobotArmTwo robotArm, double controlDT, double gravityZ,
		...
		/*
		 * YoDoubles need to be created first with a given name that is to represent the
		 * variable in the Simulation Construction Set, and the registry so the
		 * simulation can find them.
		 */
	...
	private WholeBodyControllerCore createControllerCore(double controlDT, double gravityZ,
		...
		/*
		 * We indicate in the following that we want to be able to run the 3 different
		 * control modes. As our robot has only a fixed-base and supporting rigid-body,
		 * we just provide and empty list for the contacting bodies.
		 */
		...
		/*
		 * Upon instantiation, the controller core attempts to create all the
		 * YoVariables needed for debug purpose and monitoring. For this purpose, a
		 * template of all the feedback commands that'll be needed during this control
		 * session should be provided. In this example, we only are interested in
		 * controlling the robot in jointspace, so we make a template that is about
	...
	public void initialize() {
		/*
		 * Initialize the gains. The first term is the proportional gain while the
		 * second is the damping ratio. A value of 1.0 for the damping ratio will
		 * compute a derivative gain to achieve critical damping.
		 */
	 ...
	 * <p>
	 * In this example, we will make the robot follow simple sine wave trajectories
	 * on the shoulder yaw and elbow pitch joints.
	 * </p>
	 */
	@Override
	public void doControl() {
		...
		for (SevenDoFArmJointEnum jointEnum : SevenDoFArmJointEnum.values()) {
			...
			case INVERSE_DYNAMICS:
				/*
				 * Note that for high trajectory tracking quality, you would want to define the
				 * desired acceleration, which would be used as a feed-forward term in the
				 * controller.
				 */
			...
			case VIRTUAL_MODEL:
				/*
				 * Note that for high trajectory tracking quality, you would want to define the
				 * desired torque/force, which would be used as a feed-forward term in the
				 * controller.
				 */
