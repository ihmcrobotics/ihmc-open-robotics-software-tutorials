...
/**
 * As in the {@code RobotArmOneSimulation} and {@code RobotArmTwoSimulation}
 * examples, we will use the same 7-DoF robot arm which is controlled using the
 * IHMC whole-body controller core. This time, the end-effector is controlled in
 * taskspace to follow a 3D position and orientation trajectory, see
 * {@link RobotArmThreeController}.
 * <p>
 * This example highlights the protocol for setting the controller core and the
 * possibility to run it with 3 different modes: inverse dynamics, inverse
 * kinematics, and virtual model control.
 * </p>
 * <p>
 * Even though this example applies the controller core on a simple system, the
 * procedure for controlling a much more complicated robot system remains
 * extremely similar.
 * </p>
 * <p>
...
public class RobotArmThreeSimulation {
	...
	public RobotArmThreeSimulation() {
		...
		RobotArmThreeController robotArmController = new RobotArmThreeController(robotArm, simulateDT, gravityMagnitude,
				controlMode, yoGraphicsListRegistry);
