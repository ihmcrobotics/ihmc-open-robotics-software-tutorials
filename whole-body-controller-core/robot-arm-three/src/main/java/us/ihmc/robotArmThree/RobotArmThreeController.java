...
public class RobotArmThreeController implements RobotController {
	...
	/**
	 * The desired position for the end-effector that can be monitored in the
	 * Simulation Construction Set.
	 */
	private final YoFramePoint3D desiredEndEffectorPosition = new YoFramePoint3D("desiredEndEffector", WORLD_FRAME,
			registry);
	/**
	 * The desired orientation for the end-effector that can be monitored in the
	 * Simulation Construction Set.
	 */
	private final YoFrameQuaternion desiredEndEffectorOrientation = new YoFrameQuaternion("desiredEndEffector",
			WORLD_FRAME, registry);
	/**
	 * The desired linear velocity for the end-effector that can be monitored in the
	 * Simulation Construction Set.
	 */
	private final YoFrameVector3D desiredEndEffectorLinearVelocity = new YoFrameVector3D(
			"desiredEndEffectorLinearVelocity", WORLD_FRAME, registry);
	/**
	 * The desired angular velocity for the end-effector that can be monitored in
	 * the Simulation Construction Set.
	 */
	private final YoFrameVector3D desiredEndEffectorAngularVelocity = new YoFrameVector3D(
			"desiredEndEffectorAngularVelocity", WORLD_FRAME, registry);

	// inserted
	private final YoFrameVector3D feedForwardLinearAcceleration = new YoFrameVector3D("feedForwardLinearAcceleration",
			WORLD_FRAME, registry);

	/**
	 * This time we define "SE3" which allows us to specify gains for each degree of
	 * freedom (DoF).
	 * <p>
	 * In this example, we use {@code GainCoupling.XYZ} which specify that we are
	 * going to use the same gains for the 3 position DoFs, and the 3 orientation
	 * DoFs.
	 * </p>
	 */
	private final DefaultYoPIDSE3Gains gains = new DefaultYoPIDSE3Gains("jointsGains",
			new PIDSE3Configuration(GainCoupling.XYZ, false), registry);
	 ...
	 * <ul>
	 * <li>Inverse Dynamics: Given desired accelerations and contact states, the
	 * controller core computes desired joint torques.
	 * <li>Virtual Model Control: It is a generalization of the "Jacobian transpose"
	 * method to a whole-body framework, the output is desired joint torques.
	 * <li>Inverse Kinematics: Given desired velocities, the controller core can
	 * integrate the these velocities to output both desired joint velocities and
	 * positions.
	 * </p>
	 */
	...
	/**
	 * In this controller, we add some visualization to see the end-effector desired
	 * trajectory in the Simulation Construction Set. In addition to have an
	 * interesting name, a {@code BagOfBalls} allows to easily dynamically display
	 * 3D spheres given a position.
	 */
	private final BagOfBalls trajectoryPositionVisualization;
	/**
	 * We use this field to control the rate at which a new sphere is to be
	 * displayed.
	 */
	private final YoInteger numberOfControlTicksPerVizUpdate = new YoInteger("numberOfControlTicksPerVizUpdate",
			registry);
	private final YoFramePoseUsingYawPitchRoll controlFramePoseForVisualization = new YoFramePoseUsingYawPitchRoll(
			"controlFrame", WORLD_FRAME, registry);

	/**
	 * @param robotArm               this is our simulated robot.
	 * @param controlDT              the duration of a controller tick. In this
	 *                               example, it should be equal to the simulation
	 *                               DT.
	 * @param gravityZ               the magnitude of the gravitational
	 *                               acceleration. It should be the same as the one
	 *                               used for the simulation.
	 * @param controllerCoreMode     indicates the mode for this simulation.
	 * @param yoGraphicsListRegistry in this example, we use this registry to enable
	 *                               the {@link #trajectoryPositionVisualization}.
	 */
	public RobotArmThreeController(RobotArmTwo robotArm, double controlDT, double gravityZ,
			WholeBodyControllerCoreMode controllerCoreMode, YoGraphicsListRegistry yoGraphicsListRegistry) {
		...
		trajectoryPositionVisualization = new BagOfBalls(numberOfBalls, ballSize, YoAppearance.Yellow(), registry,
				yoGraphicsListRegistry);
		numberOfControlTicksPerVizUpdate.set(300);

		/*
		 * We create here a graphical coordinate system so we can visualize where the
		 * control frame is in the simulation.
		 */
		YoGraphicCoordinateSystem controlFrameGraphic = new YoGraphicCoordinateSystem("controlFrameFrame",
				controlFramePoseForVisualization, 0.2, YoAppearance.Red());
	...
	private WholeBodyControllerCore createControllerCore(double controlDT, double gravityZ,
		...
		WholeBodyControlCoreToolbox toolbox = new WholeBodyControlCoreToolbox(controlDT, gravityZ, rootJoint,
				controlledJoints, robotArm.getCenterOfMassFrame(), controllerCoreOptimizationSettings,
		...
		/*
		 * This time, we will control the robot in taskspace by directly commanding the
		 * end-effector pose in space. We will be using a SpatialFeedbackControlCommand
		 * for this purpose.
		 */
		...
		/*
		 * In addition to switching to taskspace control, we will also use a feature of
		 * the controller core called the privileged configuration. When enabled, the
		 * controller core will attempt to keep the robot as close as possible to
		 * preferred configuration without disturbing the other objectives. This is
		 * useful for instance to use the robot redundancy to keep its joints in the
		 * middle of their range of motion.
		 */
	...
	public void initialize() {
		/*
		 * Initialize the gains. These are rather arbitrary values giving a decent
		 * tracking.
		 */
		...
		} else { // In Virtual Control Mode, there is no gravity compensation, so the gains have
					// to be much higher.
	...
	/**
	 * This method is called by the simulation every simulation tick. This is where
	 * the control part is to be implemented.
	 * <p>
	 * In this example, we will make the robot's end-effector follow a 3D trajectory
	 * composed of sine wave trajectories, see {@link #updateDesireds()}.
	 * </p>
	 */
	@Override
	public void doControl() {
		...
		SpatialFeedbackControlCommand command = new SpatialFeedbackControlCommand();
		/*
		 * We indicate that to achieve this command the controller core is to use the
		 * set of joints located between the elevator and the end-effector.
		 */
		RigidBodyBasics elevator = robotArm.getElevator();
		RigidBodyBasics endEffector = robotArm.getEndEffector();
		command.set(elevator, endEffector);
		// Then we provide the current desireds for the end-effector.
		command.setInverseDynamics(desiredEndEffectorPosition, desiredEndEffectorLinearVelocity,
				feedForwardLinearAcceleration);
		command.setInverseDynamics(desiredEndEffectorOrientation, desiredEndEffectorAngularVelocity,
				feedForwardLinearAcceleration);
		// Update the gains
		command.setGains(gains);
		/*
		 * This weight is used to prioritize this command in the optimization problem.
		 * As this command is the only one in this example, the weight value is not very
		 * important.
		 */
		command.setWeightForSolver(1.0);
		/*
		 * This time, we also have the option indicate which degrees of freedom are to
		 * be controlled. In this example, we control all the degrees of freedom.
		 */
		...
		/*
		 * Here we define exactly what part of the end-effector is meant to track our
		 * trajectory. this is done by defining the pose of what is called: the control
		 * frame. It is defined as an offset from the end-effector's body-fixed frame
		...
		/*
		 * We create a PrivilegedConfigurationCommand to indicate that this feature is
		 * desired and that we want to use it to bring the joints towards the middle of
		 * their range of motion using the robot redundancy.
		 */
		...
		for (SevenDoFArmJointEnum jointEnum : SevenDoFArmJointEnum.values()) {
			...
			if (controllerCoreMode != WholeBodyControllerCoreMode.INVERSE_KINEMATICS) { // The control mode is either
			...
			} else { // The control mode is Inverse Kinematics, so the joint output is desired
	...
	public void updateDesireds() {
		...
		{ // Let's make some 3D orientation trajectory
			...
			RotationTools.computeAngularVelocityInWorldFrameFromYawPitchRollAnglesRate(yawPitchRoll, yawPitchRollRates,
					desiredEndEffectorAngularVelocity);
		}
