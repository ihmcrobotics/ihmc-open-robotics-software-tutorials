...
public class MobileDefinition extends RobotDefinition {
	...
	public MobileDefinition() {
		...
		RigidBodyDefinition crossBarLvl1 = createCrossBar("crossBarLvl1", jointsLevel1[2], M1, L1, R1, Ixx1, Iyy1,
				Izz1);
		// sets viscous damping. It will be added on top of any torques due to joint
		// limits or a control system.
		// for this simulation, the mobile is completely passive and damping is the only
		// source of joint torque.
		jointLvl1DampingControllerDefinition
				.addJointsToControl(Stream.of(jointsLevel1).map(JointDefinition::getName).toArray(String[]::new));

		// setup next second bar of the crossbar
		RigidBodyDefinition crossBarLvl2;
		// for each point of this cross bar, attach a new gimbal joint and (smaller
		// crossbar)
		...
		for (int i = 0; i < 4; i++) {
			...
			// from the second level of 4 gimbal joints to each of their parent joints,
			// which is the first gimbal
			OneDoFJointDefinition[] jointsLevel2 = createGimbal("jointLvl2_" + i, crossBarLvl1,
					new Vector3D(xOffset, yOffset, -L1 / 2.0));
			...
			for (int j = 0; j < 4; j++) {
				...
				OneDoFJointDefinition[] jointsLevel3 = createGimbal("jointLvl3_" + i + "" + j, crossBarLvl2,
						new Vector3D(xOffset, yOffset, -L2 / 2.0));
				createRandomShape("toy_" + i + "" + j, jointsLevel3[2]);
				jointLvl3DampingControllerDefinition.addJointsToControl(
						Stream.of(jointsLevel3).map(JointDefinition::getName).toArray(String[]::new));
			}
		}

		addControllerDefinition(new ControllerCollectionDefinition().setControllerName("mobileController")
				.addControllerOutputReset().addControllerDefinitions(jointLvl1DampingControllerDefinition,
						jointLvl2DampingControllerDefinition, jointLvl3DampingControllerDefinition));
	}

	private OneDoFJointDefinition[] createGimbal(String name, RigidBodyDefinition predecessor,
			Tuple3DReadOnly jointOffset) {
		...
		jointX.setInitialJointState(new OneDoFJointState(EuclidCoreRandomTools.nextDouble(random, 0.25),
				EuclidCoreRandomTools.nextDouble(random, 0.5)));
		jointY.setInitialJointState(new OneDoFJointState(EuclidCoreRandomTools.nextDouble(random, 0.25),
				EuclidCoreRandomTools.nextDouble(random, 0.5)));
		jointZ.setInitialJointState(new OneDoFJointState(EuclidCoreRandomTools.nextDouble(random, Math.PI),
	 ...
	 */
	private RigidBodyDefinition createCrossBar(String name, JointDefinition parentJoint, double mass, double length,
		...
		// since the origin of a cylinder is the center of its base, we must first
		// translate down half the length of the vertical cylinder before adding it.
		RigidBodyTransform verticalBarPose = new RigidBodyTransform(new AxisAngle(),
				new Vector3D(0.0, 0.0, -0.25 * length));
		// position the first crossbar cylinder to point along the x axis
		RigidBodyTransform crossBarCenter1 = new RigidBodyTransform(new AxisAngle(Axis3D.X, 0.5 * Math.PI),
				new Vector3D(0.0, 0.0, -0.5 * length));
		// position the second crossbar cylinder to point along the y axis
		RigidBodyTransform crossBarCenter2 = new RigidBodyTransform(new AxisAngle(Axis3D.Y, 0.5 * Math.PI),
				new Vector3D(0.0, 0.0, -0.5 * length));
		RigidBodyTransform crossBarTip1 = new RigidBodyTransform(new AxisAngle(),
				new Vector3D(length, 0.0, -0.5 * length));
		RigidBodyTransform crossBarTip2 = new RigidBodyTransform(new AxisAngle(Axis3D.Z, -0.5 * Math.PI),
				new Vector3D());
		crossBarTip2.multiply(crossBarTip1);
		RigidBodyTransform crossBarTip3 = new RigidBodyTransform(new AxisAngle(Axis3D.Z, -0.5 * Math.PI),
				new Vector3D());
		crossBarTip3.multiply(crossBarTip2);
		RigidBodyTransform crossBarTip4 = new RigidBodyTransform(new AxisAngle(Axis3D.Z, -0.5 * Math.PI),
				new Vector3D());
