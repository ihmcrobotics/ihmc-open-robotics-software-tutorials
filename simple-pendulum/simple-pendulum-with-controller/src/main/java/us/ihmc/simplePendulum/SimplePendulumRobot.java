...
public class SimplePendulumRobot extends Robot
{
   ...
   /*
    * Some joint state variables. Allows SimplePendulumRobot to have access to and set fulcrum joint
    * properties.
    */
   ...
   {
      // Call parent class "Robot" constructor. The string "pendulum" will be the name of the robot.
       ...
       * height of 1.5 meters above the ground, the default vector (0.0, 0.0, 1.5) will be used. The third
       * parameter "this" refers to the robot itself. The final parameter "Axis3D.Y" defines the axis of
       * rotation for this pin joint.
       */
      ...
      /*
       * This line adds fulcrumPinJoint as a rootJoint of the robot. In order to be part of a robot, a
       * joint must either be added as a root joint, or be attached to a parent joint. This ensures the
       * tree structure (forest structure if there are multiple root joints) of the robot.
       */
      this.addRootJoint(fulcrumPinJoint);
   }

   /**
    * Retrieves and returns the fulcrum's angular position {@code YoDouble} in radians
    *
   ...
   /**
    * Retrieves and returns the fulcrum's angular velocity {@code YoDouble} in radians per seconds
    *
   ...
   private Link pendulumLink()
   {
      ...
      /*
       * Sets the moment of inertia about the Y axis. Note that the moment of inertia is defined about the
       * center of mass, so if set to zero, the link will be a point mass.
       */
      ...
      // Add the pivot
      /*
       * Adds a purple sphere 3D object to pendulumGraphics. Since no transformation has been applied to
       * this graphic component, the sphere's position is (0.0, 0.0, 0.0). This is used to represent the
       * pivot/fulcrum of the pendulum.
       */
