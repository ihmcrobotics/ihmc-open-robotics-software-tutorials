 ...
 *
 */
public class SimplePendulumRobot extends Robot // by extending Robot, SimplePendulumRobot is inheriting some properties and methods from the Robot class
{
   /*
   ...
   */
   ...
   /* Some joint state variables. Allows SimplePendulumRobot to have access to and set fulcrum joint properties */
   ...
   {
      /* Call parent class "Robot" constructor. The string "SimplePendulum" will be the name of the robot. */
      ...
      /** 
       * The first parameter "FulcrumPin" is the name of the joint and will be used in all the variables associated
       * with the joint. The second parameter "new Vector3d(0.0, 0.0, 1.5)" defines the offset of this joint from 
       * the previous joint. Since we want to position the fulcrum of the pendulum at a height of 1.5 meters above the ground,
       * the default vector (0.0, 0.0, 1.5) will be used. The third parameter "this" refers to the robot itself. The final
      ...
      // Sets initial state of the pin joint. The pendulum will start is course from a horizontal position with no initial speed.
      fulcrumPinJoint.setInitialState(fulcrumInitialPositionRadians, fulcrumInitialVelocity);
      // Sets the link created previously as the link for this joint (pendulumLink() method defined next).
      ...
      /** 
       * This line adds fulcrumPinJoint as a rootJoint of the robot. In order to be part of a robot, a joint 
   ...
   {
      Link pendulumLink = new Link("PendulumLink"); // creates a new link named "PendulumLink"
      
      /* Sets link's physical properties */
      // sets the moment of inertia about the Y axis. Note that the moment of inertia is defined
      // about the center of mass, so if set to zero, the link will be a point mass.
      ...
      // sets center of mass offset to be located at tip of rod, which is 1.0m opposite the pivot joint.
      ...
      /* Add the pivot */
      /**
       * Adds a purple sphere 3D object to pendulumGraphics. Since no transformation has been applied to this graphic 
       * component, the sphere's position is (0.0, 0.0, 0.0). This is used to represent the pivot/fulcrum of the pendulum.
       */
      pendulumGraphics.addSphere(FULCRUM_RADIUS, YoAppearance.BlueViolet()); 
      
      /* Add the rod */
      /** 
       * Translates from the current position by the specified distances. Graphic components 
       * added after translation will appear in the new location. The coordinate system for these 
       * translations is based on those that preceded it.
       */
      ...
      /**
       * Associates our pendulumGraphics object with the pendulumLink object and in doing so, translates
       * and rotates the graphic components to be in the same frame of reference as the pendulumLink.
       */
